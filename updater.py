...
class Updater(chainer.training.StandardUpdater):
    def __init__(self, *args, **kwargs):
        ...
        self.report_start = 0 # self.args.warmup*10 ## start reporting
    ...
    def update_core(self):
        ...
        ### generator
        # X => Y => X
        x_y = self.gen_g(losses.add_noise(x, sigma=self.args.noise))  # noise injection
        ...
        if self.args.gen_start<self._iter:
            if self.args.conditional_discriminator:
                if self.args.wgan:
                    ...
                    loss_gen_g_adv = losses.loss_func_comp(self.dis_y(F.concat([x_y,x])),1.0)
            else:
                if self.args.wgan:
        ...
        if self.args.gen_start<self._iter:
            if self.args.conditional_discriminator:
                if self.args.wgan:
                    ...
                    loss_gen_f_adv = losses.loss_func_comp(self.dis_x(F.concat([y_x,y])),1.0)
            else:
                if self.args.wgan:
        ...
        if self.args.lambda_air > 0:
            loss_air_x = self.args.lambda_air * losses.loss_range_comp(x,x_y,0.9,norm='l1')
            loss_air_y = self.args.lambda_air * losses.loss_range_comp(y,y_x,0.9,norm='l1')
        ...
        for t in range(self.args.n_critics):
            ...
            if(t<self.args.n_critics-1):
                x_y_images = random.sample(self._buffer_x.images,self.args.batch_size)
                x_y_copy = Variable(self.xp.concatenate(x_y_images))
                y_x_images = random.sample(self._buffer_y.images,self.args.batch_size)
